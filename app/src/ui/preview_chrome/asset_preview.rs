...
fn asset_preview_inner(
    ...
    match kind {
        AssetPreviewKind::Markdown => {
            ...
            view! { <article class="asset-markdown-preview min-h-0 overflow-auto rounded-xl border mei-border-default mei-surface-shell/40 p-4" inner_html=html></article> }
                .into_any()
        }
        AssetPreviewKind::Image => {
            view! {
                <div class="asset-image-preview flex min-h-0 flex-1 items-center justify-center overflow-auto rounded-xl border mei-border-default mei-surface-shell/40 p-4">
        ...
        AssetPreviewKind::Pdf => {
            view! {
                <div class="asset-pdf-preview min-h-0 flex-1 overflow-hidden rounded-xl border mei-border-default mei-surface-shell/40">
        ...
        AssetPreviewKind::HtmlDocument => {
            view! {
                <div class="asset-html-preview min-h-0 flex-1 overflow-hidden rounded-xl border mei-border-default mei-surface-shell/40">
                    <iframe
        ...
        AssetPreviewKind::Csv => {
            ...
            view! {
                <div class="asset-csv-preview grid min-h-0 flex-1 gap-2 overflow-hidden">
                    <div class="flex items-center justify-between gap-2 text-[11px] mei-text-muted">
                    ...
                    <div class="overflow-auto rounded-xl border mei-border-default mei-surface-shell/40">
                        <table class="min-w-full border-collapse text-left text-xs mei-text-primary">
                            <thead class="sticky top-0 z-[1] bg-slate-900/95">
                                <tr>
                                    <th class="sticky left-0 z-[2] whitespace-nowrap border-b border-slate-700 bg-slate-900/95 px-3 py-2 font-semibold mei-text-muted">"#"</th>
                                    {headers
                                        .iter()
                                        .enumerate()
                                        .map(|(idx, value)| {
                                            ...
                                            view! {
                                                <th class="whitespace-nowrap border-b border-slate-700 px-3 py-2 font-semibold mei-text-inverse">{title}</th>
                                            }
                                        })
                                        .collect_view()}
                                </tr>
                            ...
                            <tbody>
                                {rows
                                    ...
                                    .map(|row| {
                                        ...
                                        view! {
                                            <tr>
                                                <td class="sticky left-0 z-[1] border-b mei-border-default/80 mei-surface-panel-muted px-3 py-2 align-top mei-text-muted">{row_index}</td>
                                                {row
                                                    .iter()
                                                    .map(|cell| {
                                                        view! { <td class="border-b mei-border-default/80 px-3 py-2 align-top leading-5 mei-text-body">{cell.clone()}</td> }
                                                    })
                                                    .collect_view()}
                                            </tr>
                                        }
        ...
        AssetPreviewKind::Text => {
            view! {
                <pre class="asset-text-preview min-h-0 flex-1 overflow-auto rounded-xl border mei-border-default mei-surface-shell/40 p-4 text-xs leading-6 mei-text-primary">{source.to_string()}</pre>
            }
            .into_any()
        }
        AssetPreviewKind::Unsupported => {
            view! {
                <section class="grid min-h-0 flex-1 place-content-center gap-2 rounded-xl border border-dashed mei-border-muted mei-surface-panel-muted p-6 text-center text-sm leading-6 mei-text-muted">
                    <strong class="mei-text-inverse">"暂不支持该资源类型预览"</strong>
