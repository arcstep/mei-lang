...
const RUNTIME_TAB_OVERVIEW: &str = "overview";
const RUNTIME_TAB_JSON: &str = "json";
...
pub(crate) fn runtime_shell(
    ...
    let active_node_id = active_node
        ...
        .unwrap_or("overview-host");
    let active_tab_slug = active_tab
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .unwrap_or(RUNTIME_TAB_OVERVIEW);
    let overview_active = active_tab_slug == RUNTIME_TAB_OVERVIEW;
    let json_active = active_tab_slug == RUNTIME_TAB_JSON;
    ...
    let json_panel = runtime_json_panel(runtime_snapshot_json.unwrap_or("{}"));
    let overview_href = runtime_node_href(app_path, active_node_id, Some(RUNTIME_TAB_OVERVIEW));
    let json_href = runtime_node_href(app_path, active_node_id, Some(RUNTIME_TAB_JSON));
                ...
                <main class="main h-full min-w-0 min-h-0 overflow-hidden px-0">
                    <section class="main-pane workspace-panel workspace-panel-main min-w-0 min-h-0 flex h-full flex-col overflow-hidden px-2 py-3.5">
                            ...
                            >
                                <div class="manage-view-tabs-cluster">
                                    <div class="manage-view-tabs-group" role="presentation">
                                        <a
                                            class=if overview_active { "manage-view-tab is-active" } else { "manage-view-tab" }
                                            href=overview_href
                                            role="tab"
                                            aria-selected=if overview_active { "true" } else { "false" }
                                            data-runtime-tab=RUNTIME_TAB_OVERVIEW
                                        >
                                            <span class="manage-view-tab-label">"概览"</span>
                                        </a>
                                        <a
                                            class=if json_active { "manage-view-tab is-active" } else { "manage-view-tab" }
                                            href=json_href
                                            role="tab"
                                            aria-selected=if json_active { "true" } else { "false" }
                                            data-runtime-tab=RUNTIME_TAB_JSON
                                        >
                                            <span class="manage-view-tab-label">"原始 JSON"</span>
                                        </a>
                                    </div>
                                </div>
                            </nav>
                        ...
                        <div class="manage-tab-stage min-h-0 min-w-0 flex flex-1 flex-col overflow-hidden">
                            ...
                            <section
                                class="manage-tab-panel min-h-0 min-w-0 overflow-auto"
                                data-runtime-tab-panel=RUNTIME_TAB_JSON
                                hidden=!json_active
                            >
                                {json_panel}
                            </section>
