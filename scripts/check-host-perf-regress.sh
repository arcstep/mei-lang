...
SCENARIO_FILE="${MEI_SCENARIO_FILE:-${SCRIPT_DIR}/perf-scenarios/zhifa.json}"
LEDGER_PATH="${MEI_LEDGER_PATH:-${PROJECTS_ROOT}/docs/mei-lang/benchmarks/zhifa-perf-ledger.jsonl}"
...
echo "==> 采样当前 host perf"
node "${SCRIPT_DIR}/host-perf-sample.mjs" \
  --server-url "${BASE_URL}" \
...
node "${SCRIPT_DIR}/host-perf-report.mjs" "${REPORT_ARGS[@]}"
#!/usr/bin/env bash
