...
From mei-lang root (or any cwd):
  ./scripts/desktop/build-desktop-viewer.sh              # collect sidecars + npm run build
  ./scripts/desktop/build-desktop-viewer.sh --dev        # collect (debug) + npm run dev
  ./scripts/desktop/build-desktop-viewer.sh --skip-collect   # only tauri build/dev
...
if [[ "${SKIP_COLLECT}" != "1" ]]; then
  ...
  "${MEI_LANG_ROOT}/scripts/desktop/collect-desktop-sidecars.sh" "--${PROFILE}" "${COLLECT_EXTRA[@]+"${COLLECT_EXTRA[@]}"}"
else
  echo "==> skip collect"
fi
