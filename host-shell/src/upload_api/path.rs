...
use crate::upload_support::resolve_upload_root;

use super::types::*;

pub(super) fn resolve_upload_root_for_app(
    state: &SharedState,
    app_id: &str,
...
pub(super) fn sanitize_upload_id(raw: &str) -> Result<String, ApiError> {
...
pub(super) fn canonical_upload_root(upload_root: &Path) -> Result<PathBuf, ApiError> {
    upload_root
        .canonicalize()
        .map_err(|error| ApiError::msg(format!("upload root unavailable: {error}")))
}

pub(super) fn resolve_upload_target(upload_root: &Path, rel: &str) -> Result<PathBuf, ApiError> {
...
pub(super) fn file_name_from_upload_rel(rel: &str) -> Result<String, ApiError> {
    let rel = sanitize_upload_rel(rel)?;
    Path::new(&rel)
        .file_name()
...
pub(super) fn build_rename_target_rel(from_rel: &str, to_path: &str) -> Result<String, ApiError> {
...
pub(super) fn total_chunk_count(size_bytes: u64, chunk_size: usize) -> Result<usize, ApiError> {
    if size_bytes == 0 {
        return Err(ApiError::status(
            StatusCode::BAD_REQUEST,
            "empty upload file",
...
pub(super) fn write_chunk_session_meta(
    ...
    let bytes = serde_json::to_vec_pretty(meta)
        .map_err(|error| ApiError::msg(format!("encode upload session failed: {error}")))?;
    fs::write(upload_chunk_meta_path(session_dir), bytes)
        .map_err(|error| ApiError::msg(format!("write upload session failed: {error}")))
}

pub(super) fn read_chunk_session_meta(
    session_dir: &Path,
) -> Result<UploadChunkSessionMeta, ApiError> {
    let bytes = fs::read(upload_chunk_meta_path(session_dir))
        .map_err(|error| ApiError::msg(format!("read upload session failed: {error}")))?;
    serde_json::from_slice::<UploadChunkSessionMeta>(&bytes)
        .map_err(|error| ApiError::msg(format!("decode upload session failed: {error}")))
}

pub(super) fn list_uploaded_chunk_indexes(session_dir: &Path) -> Result<Vec<usize>, ApiError> {
    let mut out = Vec::new();
    if !session_dir.exists() {
        return Ok(out);
    }
    let entries = fs::read_dir(session_dir)
        .map_err(|error| ApiError::msg(format!("read upload session dir failed: {error}")))?;
