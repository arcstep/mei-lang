...
use super::theme::{resolve_theme, ThemeResolved};
use super::viewport::{frame_stage_style, frame_viewport_config};
use mei_lang_kernel::{
    build_runtime_resource_index, clear_prebuild_build_root_override,
    set_prebuild_build_root_override, CompiledApp, SceneContract, SceneDecl, ThemeDecl,
};
...
fn attach_host_meta_only_includes_scene_drilldown_context_when_requested() {
    let fixture_root =
        std::env::temp_dir().join(format!("mei-g10-attach-host-meta-{}", std::process::id()));
