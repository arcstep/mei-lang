...
2. `mei check` / `mei-lsp` 是作者态最重要的机器反馈；`inspect/query/runtime` 只在需要 runtime/world 事实时补充。
...
3. 跑 `mei check --app <app> --json` 看 diagnostics；需要更紧密的编辑反馈时走 `mei-lsp`。
4. 如需先理解当前 workspace 有哪些 app、发现规则和别名，再跑 `mei workspace summary --source-root <dir> --json`。
   - 这个命令更适合作为 discover/layout/semantic 路由摘要，不等于完整语义有效性校验。
5. 只有在源码里没有答案时，再跑 `mei inspect world --app <app> --json` / `mei inspect inventory --app <app> --json` / `mei inspect summary --app <app> --json`。
...
7. 最后再在外部开发工具里修改源码，并回到 `mei check` / `mei-lsp` 验证。

## 常用命令

- `mei check --app <app> --scene <scene> --json`
- `mei workspace summary --source-root <dir> --json`
- `mei inspect world --app <app> --scene <scene> --json`
- `mei inspect inventory --app <app> --scene <scene> --json`
- `mei inspect summary --app <app> --scene <scene> --json`
...
- `mei mcp describe --surface author --json`
