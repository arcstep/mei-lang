...
const SCENARIOS = [
  {
    ...
    open: `${BASE}/apps/examples/core/02-external-scene-file/layout?file=main.mei&tab=preview`,
    action: async (page) => {
      await page
  ...
  },
  {
    ...
    open: `${BASE}/apps/examples/core/02-external-scene-file/layout?file=home.mei&tab=preview`,
    action: async (page) => {
      await page
  ...
  },
  {
    ...
    open: `${BASE}/apps/spbjw/layout?file=scenes/home.mei&tab=preview`,
    action: async (page) => {
...
async function runScenario(page, scenario, waitUntilDefault) {
  ...
  const onReq = (req) => {
    ...
    if (url.includes("/layout") || url.includes("/prototype")) {
