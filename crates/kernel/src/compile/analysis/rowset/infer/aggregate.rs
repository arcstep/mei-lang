...
use super::super::build::{apply_universe, eval_rowset_with_ctx, eval_universe_labels};
use crate::compile::analysis::{
    eval_context::EvalContext,
    transforms::{
        aggregate_group_rows, aggregate_group_rows_pivot, party_year_aggregate_rows,
        sort_rows_by_field, summarize_rows,
    },
};
...
pub(super) fn eval_rowset_group_by(
    ...
    if let (Some(pivot_field), Some(pivot_columns)) = (pivot_field, pivot_columns) {
        let universe = map
            .get("universe")
            .map(|expr| eval_universe_labels(expr, datasets, group_field, ctx).unwrap_or_default())
    ...
    let mut grouped = if value_field.is_none() && agg == "count" && group_fields.len() == 1 {
        crate::compile::rowset_engine::try_group_by_count_columnar(&rows, group_field, limit)
            .or_else(|| crate::compile::rowset_engine::try_polars_group_by(&rows, group_field, agg))
            .unwrap_or_else(|| aggregate_group_rows(&rows, group_field, value_field, agg, limit))
    } else {
