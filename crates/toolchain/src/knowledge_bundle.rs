...
fn build_assets(seeds: Vec<AssetSeed>) -> Vec<KnowledgeAssetDescriptor> {
    seeds.into_iter()
        .map(|seed| KnowledgeAssetDescriptor {
            id: seed.id.to_string(),
            kind: seed.kind.to_string(),
            title: seed.title.to_string(),
            rel_path: seed.rel_path.to_string(),
...
pub fn knowledge_bundle_descriptor_for_package_root(
    ...
    let assets = match surface {
        "editor" => build_assets(editor_assets()),
        "access" => build_assets(access_assets()),
        _ => Vec::new(),
    };
    let primary_entry_ids = assets
        .iter()
        .filter(|asset| asset.kind == "skill_entry" || asset.id == "access_profile")
...
fn export_asset(
    ...
    let abs_path = package_root.join(&asset.rel_path);
...
pub fn export_knowledge_bundle_for_package_root(
    ...
    let selected = descriptor
        ...
        .filter(|asset| match topic.as_deref() {
            Some(topic_id) => asset.id == topic_id || asset.kind == topic_id,
            None => true,
        })
