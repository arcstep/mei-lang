...
use crate::mrg::client_bootstrap::read_client_bootstrap;
...
pub fn mrg_status_json(source_root: &Path, app_id: &str) -> anyhow::Result<serde_json::Value> {
    ...
    let bootstrap_scopes = registry
        ...
        .into_iter()
        .filter(|scope| read_client_bootstrap(source_root, app_id, scope.as_str()).is_some())
        .collect::<Vec<_>>();
