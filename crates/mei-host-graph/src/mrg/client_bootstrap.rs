...
pub fn build_client_bootstrap_head_fragment(
    ...
    Some(format!(
        r#"<meta name="mei-bootstrap-inlined" content="0" /><meta name="mei-bootstrap-client-revision" content="{client_revision}" /><meta name="mei-bootstrap-data-generation" content="{data_generation}" /><meta name="mei-bootstrap-scope" content="{bootstrap_scope}" /><meta name="mei-bootstrap-app-id" content="{app_id_attr}" /><meta name="mei-bootstrap-metric-count" content="{metric_count}" /><meta name="mei-bootstrap-artifact-url" content="{artifact_url}" /><script>window.__mei=window.__mei||{{}};(function(){{try{{function meta(n){{var el=document.querySelector('meta[name="'+n+'"]');return el?String(el.content||"").trim():"";}}var rev=meta("mei-bootstrap-client-revision");if(rev)window.__mei.client_revision=rev;var gen=meta("mei-bootstrap-data-generation");if(gen){{window.__mei.bootstrap_data_generation=gen;window.__mei.data_generation=gen;}}var scope=meta("mei-bootstrap-scope");if(scope)window.__mei.bootstrap_scope=scope;var appId=meta("mei-bootstrap-app-id");if(appId)window.__mei.bootstrap_app_id=appId;window.__mei.bootstrap_artifact_url="{artifact_url}";if(rev&&appId&&scope){{try{{var lsKey="mei:scene-bootstrap:v1:"+appId+":"+scope+":"+rev;var raw=localStorage.getItem(lsKey);if(raw){{var p=JSON.parse(raw);if(p.clientRevision)window.__mei.client_revision=p.clientRevision;if(p.bootstrapScope)window.__mei.bootstrap_scope=p.bootstrapScope;if(p.targetFile)window.__mei.bootstrap_target_file=p.targetFile;if(p.compileEpoch){{window.__mei.bootstrap_compile_epoch=p.compileEpoch;window.__mei.compile_epoch=p.compileEpoch;}}if(p.dataGeneration){{window.__mei.bootstrap_data_generation=p.dataGeneration;window.__mei.data_generation=p.dataGeneration;}}if(p.appId)window.__mei.bootstrap_app_id=p.appId;if(Array.isArray(p.metrics))window.__mei.bootstrap_metrics=p.metrics;if(Array.isArray(p.bootstrapScopes))window.__mei.bootstrap_scopes=p.bootstrapScopes;window.__meiBootstrapPayloadReady=1;window.__meiBootstrapFromLocalStorage=1;window.__meiEvalPackSource="scene_bootstrap_local";}}}}catch(e){{}}}}}}catch(e){{window.__meiBootstrapSeedError="bootstrap_meta_failed";}}try{{if(window.__meiBootstrapPayloadReady)document.dispatchEvent(new CustomEvent("mei-bootstrap-ready"));}}catch(e){{}}}})();</script>"#
    ))
}
