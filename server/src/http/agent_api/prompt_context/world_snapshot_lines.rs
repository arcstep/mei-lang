...
pub(super) fn append_world_context_snapshot_lines(
    ...
    lines.push(String::new());
    lines.push("[World — compact]".to_string());
    lines.push(format!(
    ...
    lines.push(format!(
        "resource_inventory: related_items below are file/scene hints; [World — catalog] above is authoritative for world.resources ids. Query contracts are listed under [World — query tooling]. For dataset resources, use dataset_query(id) for rows/schema and dataset_metric(id) for metric values."
    ));
}
