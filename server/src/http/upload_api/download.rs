...
pub(super) fn content_disposition_attachment(file_name: &str) -> Result<HeaderValue, AppError> {
...
pub(super) fn content_disposition_inline(file_name: &str) -> Result<HeaderValue, AppError> {
    let ascii_name = ascii_content_disposition_filename(file_name);
    let encoded = percent_encode_for_content_disposition(file_name);
        ...
        *response.status_mut() = StatusCode::PARTIAL_CONTENT;
        ...
        response.headers_mut().insert(
            ...
            HeaderValue::from_str(&format!("bytes {start}-{end}/{file_len}"))
                .map_err(|error| AppError::msg(format!("invalid content range: {error}")))?,
        );
        response.headers_mut().insert(
