...
pub(super) fn resolve_upload_file_ignore_name_case(
    ...
    let file_name = path
        ...
        .ok_or_else(|| AppError::msg(format!("upload file not found: {rel}")))?;
    let parent_rel = path
        .parent()
        .and_then(|value| value.to_str())
        .unwrap_or("")
    ...
    let dir_path = if parent_rel.is_empty() {
        ...
        let canonical_dir = joined.canonicalize().map_err(|error| {
            AppError::msg(format!("upload file not found: {rel} ({error})"))
        })?;
        ...
        if !canonical_dir.is_dir() {
            return Err(AppError::msg(format!("upload file not found: {rel}")));
        }
        canonical_dir
    };
    ...
    {
        let entry =
            entry.map_err(|error| AppError::msg(format!("failed to read upload entry: {error}")))?;
