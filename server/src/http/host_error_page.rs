...
pub fn render_auth_card_page(document_title: &str, headline: &str, card_inner: &str) -> String {
    shell_layout(document_title, None, headline, card_inner, "")
}
...
mod tests {
    ...
    fn auth_card_page_omits_status_code() {
        let html = render_auth_card_page("登录 - MeiLang", "MeiLang 登录", "<p>form</p>");
        assert!(!html.contains("mei-host-shell__status"));
        assert!(html.contains("mei-host-shell__coin"));
    }
}
